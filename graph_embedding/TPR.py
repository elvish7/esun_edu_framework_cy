...
class TPR:
    ...
    def fit(self, d=128, ns=5, lr=0.025, update_times=100):
        CMD = f'./codes.tpr.rec/tpr -train_ui {self.input_ui_path} -train_iw {self.input_iw_path} -save /dev/stderr -sample_times {update_times} -dimensions {d} -negative_samples {ns} -init_alpha {lr} -threads {multiprocessing.cpu_count()}'
