...
class SMORe:
    ...
    def interactions(self):
        ...
        temp = open('input.txt', 'w') 
        for record, count in collections.Counter(data).items():
            #print(*record, count, file=input_data)
            print(*record, count, file=temp)
        #return input_data
        
    def fit(self, mode='warp', d=128, ns=5, lr=0.025, update_times=100):
        #CMD = f'./smore/cli/{mode} -train /dev/stdin -save /dev/stderr -dimensions {d} -negative_samples {ns} -alpha {lr} -threads {multiprocessing.cpu_count()}'
        ...
        CMD = f'./Graph_module/smore/cli/{mode} -train {self.input_path} -save /dev/stderr -sample_times {update_times} -dimensions {d} -negative_samples {ns} -alpha {lr} -threads {multiprocessing.cpu_count()}'
