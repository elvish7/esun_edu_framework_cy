...
def old_cust_CF_baseline(span, today = '2019-06-30', mode='all'):
    """
    說明: 
    ...
    [JH] add the span setting for training.
    ...
    # [JH] Setting spans
    if span <= 3:
        exclude_start_dt = today
        dig_start_dt = today
    else:
        exclude_start_dt = before_3m_dt
        dig_start_dt = before_3m_dt

    before_span_dt = get_data_start_dt(today, span)
    ...
    if mode == 'all':
        ...
        filter_dataloader.config(
            rawdata_conn=rawdata_conn,
            txn_exclude_start_dt=exclude_start_dt, # cust_txn_exclude
