...
def main(args):
    ...
    # get prediction
    # Note that this csv should be 'renamed' by this user/item mapping, for example
    # df.rename(columns={"A": "a", "B": "c"})
    # df.rename(index={0: "x", 1: "y", 2: "z"})

    pred_df = pd.read_csv(args.path_prediction_csv)
    pred_df = pd.DataFrame(pred_df.values.argsort().argsort())
    pred_df = pred_df.reindex(user_id_mapping.values(), fill_value=-1)
    pred_df = pred_df.reindex(columns=item_id_mapping.values(), fill_value=-1)
    pred = pred_df.values
    ...
    row_idx_list, col_idx_list = np.nonzero(pred_df.values + 1)
    ...
    with open(args.path_rec_output, 'w') as f:
        for (uid, iid, i, j) in zip(user_ids, item_ids, row_idx_list, col_idx_list):
            ...
            f.write(f'{uid}\t{iid}\t{types}\t{scores}\t{ranks}\n')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--path_prediction_csv', type=str)
